"use client";

import { useEffect, useState, useMemo } from "react";
import { useParams } from "next/navigation";
import Link from "next/link";
import Image from "next/image";

type Player = {
  clubId: string;
  clubName: string;
  lat: number;
  lon: number;
  academyClassScore: number;
  firstName: string;
  lastName: string;
  birthDate: string;
  gender: string | null;
  shirtNumber: string;
  position: string;
  competition: string;
  competitionId: string;
  ageGroupName: string;
  ageGroupId: number;
  ageGroupMaxAge: number;
  sportsCategory: string;
  photoPath: string;
};

type MatchSheet = {
  matchId: string;
  date: string;
  competitionName: string;
  clubSide: string;
  clubName: string;
  playerId: string;
  firstName: string;
  lastName: string;
  shirtNo: number;
  position: string;
  isCaptain: boolean;
  groupNo: number | null;
  photo: string;
  role: string;
};

type Match = {
  matchId: string;
  date: string;
  competitionName: string;
  homeClubId: string;
  homeClubName: string;
  awayClubId: string;
  awayClubName: string;
  homeGoals: number | null;
  awayGoals: number | null;
  stadiumName: string;
  stadiumTown: string;
};

// Curăță encoding-ul corupt din poziții
function cleanPosition(pos: string): string {
  const map: Record<string, string> = {
    "Funda\u0219 central": "Fundaș central",
    "Funda\u0219 st\u00e2nga": "Fundaș stânga",
    "Mijloca\u0219 central": "Mijlocaș central",
    "Funda\u00fe central": "Fundaș central",
    "Funda\u00fe st\u00e2nga": "Fundaș stânga",
    "Mijloca\u00fe central": "Mijlocaș central",
  };
  return map[pos] || pos;
}

function getAge(birthDate: string): number {
  if (!birthDate) return 0;
  const year = parseInt(birthDate.substring(0, 4));
  return 2026 - year;
}

function getBirthYear(birthDate: string): string {
  if (!birthDate) return "—";
  return birthDate.substring(0, 4);
}

export default function JucatorPage() {
  const params = useParams();
  const slug = decodeURIComponent(params.slug as string);
  const nameParts = slug.split("-");

  const [players, setPlayers] = useState<Player[]>([]);
  const [matchSheets, setMatchSheets] = useState<MatchSheet[]>([]);
  const [matches, setMatches] = useState<Match[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([
      fetch("/data/players.json?v=2").then((r) => r.json()),
      fetch("/data/match_sheets.json?v=2").then((r) => r.json()),
      fetch("/data/matches.json?v=2").then((r) => r.json()),
    ]).then(([p, ms, m]) => {
      // Filtrează doar juniori
      const juniorKeywords = /U\d|Juniori|Junioare|Liga Elitelor|Campionatul Național|Liga de Tineret|Interliga/i;
      setPlayers(p.filter((pl: Player) => juniorKeywords.test(pl.competition)));
      setMatchSheets(ms);
      setMatches(m);
      setLoading(false);
    });
  }, []);

  // Găsește jucătorul după nume (slug-ul e "firstName-lastName")
  const playerRecords = useMemo(() => {
    if (!players.length) return [];
    const firstName = nameParts[0] || "";
    const lastName = nameParts.slice(1).join(" ") || "";
    const fullName = slug.replace(/-/g, " ").toLowerCase().trim();

    // 1. Match exact pe firstName + lastName (cu trim)
    let found = players.filter(
      (p) =>
        p.firstName.trim().toLowerCase() === firstName.toLowerCase() &&
        p.lastName.trim().toLowerCase() === lastName.toLowerCase()
    );
    // 2. Match pe numele complet exact
    if (found.length === 0) {
      found = players.filter(
        (p) => (p.firstName.trim() + " " + p.lastName.trim()).toLowerCase() === fullName
      );
    }
    // 3. Match parțial (includes)
    if (found.length === 0) {
      found = players.filter((p) =>
        (p.firstName.trim() + " " + p.lastName.trim()).toLowerCase().includes(fullName)
      );
    }
    return found;
  }, [players, slug, nameParts]);

  // Date de bază ale jucătorului (din prima înregistrare)
  const player = playerRecords[0];

  // Meciurile jucătorului din match_sheets
  const playerMatches = useMemo(() => {
    if (!matchSheets.length || !player) return [];
    return matchSheets
      .filter(
        (ms) =>
          ms.firstName.trim() === player.firstName.trim() &&
          ms.lastName.trim() === player.lastName.trim()
      )
      .sort((a, b) => b.date.localeCompare(a.date));
  }, [matchSheets, player]);

  // Poza reală din match sheets
  const realPhoto = useMemo(() => {
    const withPhoto = playerMatches.find(
      (ms) => ms.photo && ms.photo !== "null" && ms.photo.startsWith("http")
    );
    return withPhoto?.photo || null;
  }, [playerMatches]);

  // Map meciuri după matchId pentru detalii
  const matchDetails = useMemo(() => {
    const map: Record<string, Match> = {};
    matches.forEach((m) => { map[m.matchId] = m; });
    return map;
  }, [matches]);

  // Statistici
  const stats = useMemo(() => {
    if (!playerMatches.length) return null;
    const titular = playerMatches.filter((m) => m.role === "titular").length;
    const rezerva = playerMatches.filter((m) => m.role === "rezervă").length;
    const rezervaExtra = playerMatches.filter((m) => m.role === "rezervă extra").length;
    const captain = playerMatches.filter((m) => m.isCaptain).length;
    const positions = [...new Set(playerMatches.map((m) => cleanPosition(m.position)))];
    const competitions = [...new Set(playerMatches.map((m) => m.competitionName))];
    const clubs = [...new Set(playerMatches.map((m) => m.clubName))];

    // Rezultate meciuri (W/D/L)
    let wins = 0, draws = 0, losses = 0;
    playerMatches.forEach((ms) => {
      const match = matchDetails[ms.matchId];
      if (!match || match.homeGoals === null || match.awayGoals === null) return;
      const isHome = ms.clubSide === "home";
      const myGoals: number = isHome ? match.homeGoals : match.awayGoals;
      const oppGoals: number = isHome ? match.awayGoals : match.homeGoals;
      if (myGoals > oppGoals) wins++;
      else if (myGoals < oppGoals) losses++;
      else draws++;
    });

    return {
      total: playerMatches.length,
      titular,
      rezerva,
      rezervaExtra,
      captain,
      positions,
      competitions,
      clubs,
      wins,
      draws,
      losses,
    };
  }, [playerMatches, matchDetails]);

  if (loading) {
    return (
      <div className="pt-32 pb-20">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <p className="text-muted">Se încarcă datele jucătorului...</p>
        </div>
      </div>
    );
  }

  if (!player) {
    return (
      <div className="pt-32 pb-20">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h1 className="font-display text-3xl font-bold mb-4">Jucător negăsit</h1>
          <p className="text-muted mb-6">Nu am găsit jucătorul: {slug}</p>
          <Link href="/jucatori" className="text-violet hover:underline">
            ← Înapoi la lista de jucători
          </Link>
        </div>
      </div>
    );
  }

  const age = getAge(player.birthDate);

  return (
    <div className="pt-28 pb-20">
      <div className="max-w-5xl mx-auto px-6">
        {/* Breadcrumb */}
        <div className="mb-6">
          <Link href="/jucatori" className="text-muted hover:text-violet text-sm transition-colors">
            ← Înapoi la jucători
          </Link>
        </div>

        {/* Header card */}
        <div className="card-navy-glow p-8 mb-8">
          <div className="flex flex-col md:flex-row gap-8">
            {/* Photo */}
            <div className="flex-shrink-0 mx-auto md:mx-0">
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-2xl overflow-hidden border border-white/10 bg-navy-light">
                {realPhoto ? (
                  <Image
                    src={realPhoto}
                    alt={`${player.firstName} ${player.lastName}`}
                    width={160}
                    height={160}
                    className="w-full h-full object-cover"
                    unoptimized
                  />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-4xl font-display font-bold text-violet">
                    {player.firstName.charAt(0)}
                  </div>
                )}
              </div>
            </div>

            {/* Info */}
            <div className="flex-1 text-center md:text-left">
              <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-2">
                {player.firstName} {player.lastName}
              </h1>
              <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
                <span className="px-3 py-1 rounded-full text-xs font-semibold bg-violet/10 text-violet">
                  {player.position}
                </span>
                <span className="px-3 py-1 rounded-full text-xs font-semibold bg-green/10 text-green">
                  {age} ani
                </span>
                <span className="px-3 py-1 rounded-full text-xs font-semibold bg-amber/10 text-amber">
                  #{player.shirtNumber}
                </span>
                {player.gender && (
                  <span className="px-3 py-1 rounded-full text-xs font-semibold bg-pink/10 text-pink">
                    {player.gender === "M" ? "Masculin" : "Feminin"}
                  </span>
                )}
              </div>

              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-muted block">Club</span>
                  <span className="text-foreground font-medium">{player.clubName}</span>
                </div>
                <div>
                  <span className="text-muted block">An naștere</span>
                  <span className="text-foreground font-medium">{getBirthYear(player.birthDate)}</span>
                </div>
                <div>
                  <span className="text-muted block">Competiție</span>
                  <span className="text-foreground font-medium">{player.competition}</span>
                </div>
                <div>
                  <span className="text-muted block">Categorie</span>
                  <span className="text-foreground font-medium">{player.sportsCategory}</span>
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* Stats grid */}
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="card-navy p-5 text-center">
              <div className="text-3xl font-display font-bold text-foreground">{stats.total}</div>
              <div className="text-xs text-muted uppercase tracking-wider mt-1">Meciuri</div>
            </div>
            <div className="card-navy p-5 text-center">
              <div className="text-3xl font-display font-bold text-green">{stats.titular}</div>
              <div className="text-xs text-muted uppercase tracking-wider mt-1">Titular</div>
            </div>
            <div className="card-navy p-5 text-center">
              <div className="text-3xl font-display font-bold text-amber">{stats.rezerva}</div>
              <div className="text-xs text-muted uppercase tracking-wider mt-1">Rezervă</div>
            </div>
            <div className="card-navy p-5 text-center">
              <div className="text-3xl font-display font-bold text-violet">{stats.captain}</div>
              <div className="text-xs text-muted uppercase tracking-wider mt-1">Căpitan</div>
            </div>
          </div>
        )}

        {/* Results W/D/L */}
        {stats && stats.wins + stats.draws + stats.losses > 0 && (
          <div className="card-navy p-6 mb-8">
            <h3 className="font-display text-lg font-semibold text-foreground mb-4">
              Rezultate meciuri
            </h3>
            <div className="flex gap-6">
              <div className="text-center">
                <div className="text-2xl font-display font-bold text-green">{stats.wins}</div>
                <div className="text-xs text-muted">Victorii</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-display font-bold text-amber">{stats.draws}</div>
                <div className="text-xs text-muted">Egaluri</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-display font-bold text-red">{stats.losses}</div>
                <div className="text-xs text-muted">Înfrângeri</div>
              </div>
            </div>
          </div>
        )}

        {/* Positions played */}
        {stats && stats.positions.length > 0 && (
          <div className="card-navy p-6 mb-8">
            <h3 className="font-display text-lg font-semibold text-foreground mb-4">
              Poziții jucate
            </h3>
            <div className="flex flex-wrap gap-2">
              {stats.positions.map((pos) => (
                <span
                  key={pos}
                  className="px-4 py-2 rounded-full text-sm font-medium border border-white/8 bg-navy-light text-foreground"
                >
                  {cleanPosition(pos)}
                </span>
              ))}
            </div>
          </div>
        )}

        {/* Competitions */}
        {stats && stats.competitions.length > 0 && (
          <div className="card-navy p-6 mb-8">
            <h3 className="font-display text-lg font-semibold text-foreground mb-4">
              Competiții
            </h3>
            <div className="flex flex-wrap gap-2">
              {stats.competitions.map((comp) => (
                <span
                  key={comp}
                  className="px-4 py-2 rounded-full text-sm font-medium border border-violet/20 bg-violet/5 text-violet"
                >
                  {comp}
                </span>
              ))}
            </div>
          </div>
        )}

        {/* Match history */}
        {playerMatches.length > 0 && (
          <div className="mb-8">
            <h2 className="font-display text-2xl font-bold text-foreground mb-4">
              Istoric meciuri ({playerMatches.length})
            </h2>
            <div className="space-y-2">
              {playerMatches.map((ms, i) => {
                const match = matchDetails[ms.matchId];
                const opp = match
                  ? ms.clubSide === "home"
                    ? match.awayClubName
                    : match.homeClubName
                  : "—";
                const score = match && match.homeGoals !== null
                  ? `${match.homeGoals} - ${match.awayGoals}`
                  : "—";
                const result = match && match.homeGoals !== null && match.awayGoals !== null
                  ? (() => {
                      const isHome = ms.clubSide === "home";
                      const my: number = isHome ? match.homeGoals : match.awayGoals;
                      const oppG: number = isHome ? match.awayGoals : match.homeGoals;
                      if (my > oppG) return "W";
                      if (my < oppG) return "L";
                      return "D";
                    })()
                  : null;

                const resultColor =
                  result === "W" ? "text-green" :
                  result === "L" ? "text-red" :
                  result === "D" ? "text-amber" : "text-muted";

                return (
                  <div key={i} className="card-navy p-4 flex items-center gap-4">
                    {/* Result badge */}
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${
                      result === "W" ? "bg-green/10 text-green" :
                      result === "L" ? "bg-red/10 text-red" :
                      result === "D" ? "bg-amber/10 text-amber" : "bg-navy-light text-muted"
                    }`}>
                      {result || "—"}
                    </div>

                    {/* Date */}
                    <div className="text-xs text-muted w-16 flex-shrink-0">
                      {new Date(ms.date).toLocaleDateString("ro-RO", { day: "2-digit", month: "short" })}
                    </div>

                    {/* Role */}
                    <div className="w-20 flex-shrink-0">
                      <span className={`text-xs font-semibold ${
                        ms.role === "titular" ? "text-green" :
                        ms.role === "rezervă" ? "text-amber" : "text-muted"
                      }`}>
                        {ms.role === "titular" ? "Titular" : ms.role === "rezervă" ? "Rezervă" : "Rez. extra"}
                      </span>
                      {ms.isCaptain && (
                        <span className="block text-xs text-violet">C</span>
                      )}
                    </div>

                    {/* Position */}
                    <div className="w-32 flex-shrink-0 text-xs text-foreground truncate hidden md:block">
                      {cleanPosition(ms.position)}
                    </div>

                    {/* Opponent */}
                    <div className="flex-1 text-sm text-foreground truncate">
                      vs {opp}
                    </div>

                    {/* Score */}
                    <div className={`text-sm font-bold ${resultColor} flex-shrink-0`}>
                      {score}
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        )}

        {/* No matches */}
        {!stats && (
          <div className="card-navy p-8 text-center">
            <p className="text-muted">
              Acest jucător nu apare în foile de meci. Doar datele de bază din lot
              sunt disponibile.
            </p>
          </div>
        )}

        {/* Multiple clubs note */}
        {playerRecords.length > 1 && (
          <div className="card-navy p-6 mt-8">
            <h3 className="font-display text-lg font-semibold text-foreground mb-3">
              Înscris la {playerRecords.length} competiții
            </h3>
            <div className="space-y-2">
              {playerRecords.map((r, i) => (
                <div key={i} className="flex justify-between text-sm">
                  <span className="text-foreground">{r.competition}</span>
                  <span className="text-muted">{r.clubName} · #{r.shirtNumber}</span>
                </div>
              ))}
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
